...
class DataLoaderSparkOOT(BaseDataLoader):
    ...
    validate_data = None
    sample_ratio: float
    def __init__(self, cutoff_date:datetime, datetime_col:str, sample_ratio:float =1.0) -> None:
        super().__init__()
    ...
    def get_train_data(self):
        ...
        df_train = self.df.filter(
            F.col(self.datetime_col)<self.cutoff_date
        )
...
class DataLoaderSparkRecommendOOT(DataLoaderSparkOOT):
    ...
    def __init__(self, cutoff_date: datetime, datetime_col: str, user_identity:str, target:str,groundtruth_col:str,sample_ratio:float =1.0) -> None:
    ...
    def get_validate_data(self):
        ...
        df_eval_return = df_eval.groupBy(
            self.user_identity
        ).agg(
            F.collect_set(self.target).alias(self.groundtruth_col)
        ).join(
            self.user_common,on=self.user_identity
        )
    ...
    def get_train_data(self):
        ...
        df_user_common = df_train.select(self.user_identity).drop_duplicates().join(
