    ...
    def evaluate(self,df_recommend: SparkDataFrame, df_groundtruth: SparkDataFrame):
        ...
        df_get_hit_up_to_k = (
            ...
            .withColumn(
                ...
                F.size(F.array_intersect(F.col("rec_up_to_k"), F.col(self.groundtruth_col))),
            )
            .filter(F.col("no_relevance_up_to_k") > 0)
        )
        window_get_hit = (
            Window().partitionBy(self.user_identity, "no_relevance_up_to_k").orderBy("up_to_k")
        )
        ...
        df_apk_user = (
            df_precision_up_to_k.withColumn("len_grouth_truth", F.size(F.col(self.groundtruth_col)))
        ...
        df_apk_user = (
            ...
            .join(df_groundtruth.select(self.user_identity, self.groundtruth_col), on=self.user_identity)
            .join(df_recommend.select(self.user_identity, self.rec_col), on=self.user_identity)
        )
